...
namespace Random {
	...
	void Init(const unsigned int seed = std::random_device{}());
    ...
    const int GetWeightedIndex(const std::vector<double>& weights);
    ...
    const double GetNormal(const double mean, const double stddev);
    ...
    const bool GetBernoulli(const double p);
    ...
    /// <returns>乱数</returns>
    const double GetExponential(const double lambda);

	/// <summary>
	/// 現在のシードを獲得
	/// </summary>
	/// <returns>現在のシード値</returns>
    const unsigned int GetSeed();// 現在のシード値を取得
};