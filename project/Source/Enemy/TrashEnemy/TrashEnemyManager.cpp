...
void TrashEnemyManager::Update()
{
	...
	{
		//òAågçUåÇÇÃÇ∆Ç´Ç…ÇªÇÃìGÇ™èÄîıäÆóπÇµÇΩÇ©Ç«Ç§Ç©
		if ((*itr)->GetStandby())
			counter++;
		//ìGëSàıÇ™èÄîıäÆóπÇµÇΩÇÁçUåÇÇ…à⁄ÇÈ
		...
		//éGãõìGÇ™éÄÇÒÇ≈ÇΩÇÁlistÇ©ÇÁçÌèúÇ∑ÇÈ
		if (!(*itr)->GetActive())
		{
			(*itr)->GetEnemyObj()->DestroyMe();
			//éüÇÃóvëfÇÃÉCÉeÉåÅ[É^Ç™ï‘ÇÈ
...
void TrashEnemyManager::CreateEnemy(VECTOR3 _pos, float enemySpawnCounter)
{
    for (int i = 0; i < enemySpawnCounter; i++)
    {
		...
		//ìñÇΩÇËîªíËÇê∂ê¨ÅiÇ‚ÇÁÇÍîªíËÅj
        ...
        // å¬ï ÇÃTrashEnemyÇí«â¡
        TrashEnemy* t = e->Component()->AddComponent<TrashEnemy>();
		t->Start(e);
        // enemiesÇ…ìoò^Åiå¬ï ÉCÉìÉXÉ^ÉìÉXÅj
