...
void StageManager::CreateWall()
{
	...
	for (int i = 0; i < 4; i++) {
		...
		{
		case 0:
			transform = Transform(VECTOR3(13000, 100, 100), VZero, VOne* VECTOR3(28.0f, 50, 120.0f));
			break;
		case 1:
			transform = Transform(VECTOR3(-13000, 100, 100), VZero, VOne * VECTOR3(28.0f, 50, 120.0f));
			break;
		case 2:
			transform = Transform(VECTOR3(0, 100, -13000), VZero, VOne * VECTOR3(120.0f, 50, 28.0f));
			break;
		case 3:
			transform = Transform(VECTOR3(0, 100, 13000), VZero, VOne * VECTOR3(120.0f, 50, 28.0f));
			break;
...
{
	//ToDO なぜかfor文で回さないと貫通してしまうのでfor文を使わなくてもいいようにする。
	//for (int i = 0; i < 4; i++) {
	Transform transform;
